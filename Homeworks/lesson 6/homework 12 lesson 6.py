"""
Задание
Изучите через консоль разработчика в браузере следующие сайты на предмет
наличия скрытых API:
...
● https://fedresurs.ru/
● https://kad.arbitr.ru/
Попробуйте обратиться к ним для получения данных. Оставьте комментарий в вашем
репозитори"""
...
"""
# Найденные скрытые API

1. **Nasdaq**:
   - Endpoint: https://api.nasdaq.com/api/quote/AAPL/info
   - Пример использования: get_nasdaq_data()

2. **LME**:
   - Endpoint: https://www.lme.com/api/listings
   - Пример использования: get_lme_data()

3. **5ka.ru**:
   - Endpoint: https://api.5ka.ru/api/v2/special_offers/
   - Пример использования: get_5ka_data()

4. **Fedresurs**:
   - Endpoint: https://fedresurs.ru/backend/api/some_endpoint
   - Пример использования: get_fedresurs_data()

5. **Kad.arbitr.ru**:
   - Endpoint: https://kad.arbitr.ru/Kad/SearchInstances
   - Пример использования: get_kad_data()"""